...
public class GameService {
    ...
    public GameResponse getGameById(String id) {
        Game game = gameRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.PRODUCT_NOT_EXISTED));
    ...
    public String updateGameImage(String gameId, String imageUrl) {
        ...
        log.info("Updated image for game {}: {}", gameId, imageUrl);
        return imageUrl;
    }

    // Update game cover URL
    public String updateGameCover(String gameId, String coverUrl) {
        Game game = gameRepository.findById(gameId)
                .orElseThrow(() -> new AppException(ErrorCode.PRODUCT_NOT_EXISTED));
        ...
        log.info("Updated cover for game {}: {}", gameId, coverUrl);
        return coverUrl;
    }

    // Update game video URL
    public String updateGameVideo(String gameId, String videoUrl) {
        Game game = gameRepository.findById(gameId)
                .orElseThrow(() -> new AppException(ErrorCode.PRODUCT_NOT_EXISTED));

        game.setVideo(videoUrl);
        gameRepository.save(game);

        log.info("Updated video for game {}: {}", gameId, videoUrl);
        return videoUrl;
    }
}
